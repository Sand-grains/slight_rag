...
def chunk(docs: List[Chunk], chunk_size: int = CHILD_CHUNK_SIZE, chunk_overlap: int = CHILD_OVERLAP) -> List[Chunk]:
    """滑动窗口切分：将每篇文档按固定窗口大小切为多个有重叠的 chunk, chunk_id 确定性生成"""
    chunks = []
    for doc in docs:
        text = doc.content
        if len(text) <= chunk_size:
            origin_metadata = deepcopy(doc.origin_metadata)
            origin_metadata.chunk_index = 0
        ...
        while start < len(text):
            ...
            origin_metadata = deepcopy(doc.origin_metadata)
            origin_metadata.chunk_index = idx
            chunks.append(replace(doc,
                content=chunk_text,
