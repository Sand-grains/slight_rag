...
核心特性：
    ...
    - 构建"坏答案"：SWAP_TIER_1（同级术语替换）+ SWAP_TIER_2（结构破坏），词表无匹配时兜底到 bad_b（错误 chunk 生成的整段幻觉答案）
...
def _make_hallucination(good_answer: str, use_tier2: bool = False) -> tuple[str, bool]:
    ...
    Returns:
        (bad, replaced)：replaced 表示词表是否有命中。
        无命中时不兜底，由调用方 fallback 到 bad_b（错误 chunk 生成的整段幻觉答案）——
        元说明兜底是弱负样本（good 的 claims 仍与 context 一致，faithfulness 按 rubric 应给高分）。
    """
...
def run_calibrate() -> dict:
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        for item in samples:
            ...
            # 坏答案 2: 错误 chunk 喂入（先生成，供 bad_a 兜底引用）
            wrong_context = _inject_wrong_chunks(store, item.query, retriever)
            bad_b = generator.generate(item.query, wrong_context)

            # 坏答案 1: 幻觉注入（词表无命中时兜底到 bad_b）
            bad_a, replaced = _make_hallucination(answer, use_tier2=use_tier2)
            if not replaced:
                bad_a = bad_b
            print(f"    bad (幻觉注入):\n{bad_a}\n")
            ...
            jr_good = run_judge(item.query_id, item.query, chunks, answer, item.reference_facts, skip_cache=True)
            jr_bad_a = run_judge(item.query_id, item.query, chunks, bad_a, item.reference_facts, skip_cache=True)
            # bad_b 由错误 chunk 生成、用正确 chunks 评 → claims 无支撑 → faithfulness 真正降低
            jr_bad_b = run_judge(item.query_id, item.query, chunks, bad_b, item.reference_facts, skip_cache=True)
            ...
            records.append({
                ...
                "good": {"faithfulness": jr_good.faithfulness, "answer_correctness": jr_good.answer_correctness,
                         ...
                         "judge_error": jr_good.judge_error},
                "bad_a": {"faithfulness": jr_bad_a.faithfulness, "answer_correctness": jr_bad_a.answer_correctness,
                          ...
                          "judge_error": jr_bad_a.judge_error},
                "bad_b": {"faithfulness": jr_bad_b.faithfulness, "answer_correctness": jr_bad_b.answer_correctness,
                          ...
                          "judge_error": jr_bad_b.judge_error},
            })
        ...
        for r in records:
            g = r["good"]
            if g["verdict"] == "fail":
                if g["judge_error"] or g["parse_error"]:
                    good_judge_error.append(r["query_id"])
                else:
                    good_verdict_fail.append(r["query_id"])
